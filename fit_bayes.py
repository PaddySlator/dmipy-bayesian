...
def fit_bayes(model, acq_scheme, data, mask=None, nsteps=1000, burn_in=500):
    ...
    gibbs_mu = np.zeros((nparams-1, nsteps))                # track mu at each step
    gibbs_sigma = np.zeros((nparams-1, nparams-1, nsteps))  # track sigma at each step
    ...
    # loop over ROIs
    for r in range(nroi):
        idx_roi = [xx for xx, x in enumerate(mask == roi_vals[r]) if x]  # indices into mask of voxels in ROI
...
#                for p in ['C1Stick_1_lambda_par', 'G1Ball_1_lambda_iso', 'partial_volume_0', 'C1Stick_1_mu']:  # parameters_to_fit:  # [2, 3, 4, 0, 1]:  # range(nparams-1):
                for p in parameters_to_fit:
                    ...
                    if 'partial_volume_' in p:
                        f_indep = [params_all_new[name][i] for name in model.partial_volume_names if name != dependent_fraction]
                        f_indep = np.exp(f_indep) / (1 + np.exp(f_indep))       # tform indept fractions (log -> orig)
                        f_indep = [1 if np.isnan(x) else x for x in f_indep]    # check transform
                        f_dept = 1 - np.sum(f_indep)                            # compute dept fraction
                        f_dept = np.log(f_dept) - np.log(1 - f_dept)            # tform dept fraction (orig -> log)
                        params_all_new[dependent_fraction][i] = f_dept
                    ...
                    acceptance_rate[p][j] = accepted[p] / ((j+1) * nvox_roi)
